...
router.get( '/:category/:product_id', function( req, res, next ){
	var category = req.params.category;
	var product = req.params.product_id;
	db.query( 'SELECT * FROM products WHERE category  = ? and product_id = ?', [category, product], function(err, results, fields){
		if(err) throw err;
		var products = results[0];
		var header = results[0].product;
		res.render('product_description', {title: header, products: products});
	});
});
//test
...
// get how it works
router.get('/addproduct', ensureLoggedIn( '/login' ), function (req, res, next){
	//check if the user is a seller
	var currentUser = req.session
	console.log( currentUser );
	db.query( 'SELECT user_id FROM sellers WHERE user_id = ?', [currentUser], function ( err, results, fields ){
		if( err ) throw err;
		if( results.length === 0 ){
			res.redirect( '/' );
		}
			...
			db.query( 'SELECT category FROM category', function ( err, results, fields ){
				...
				res.render( 'addproduct', {title: 'ADD PRODUCTS', category: category});
			});
		}
	});
  res.render('addproduct', {titlle: " ADD PRODUCT"});
}); 
